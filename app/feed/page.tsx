...
// REMOVE: import BottomNav from '../components/Navigation/BottomNav';
...
export default function FeedPage() {
  ...
  const handleDeletePost = async (postId: string) => {
    const success = await feed.deletePost(postId);
    if (success) {
  ...
  const handleUpdatePost = async (content: string, link: string | undefined) => {
    if (!editingPost) return false;
    return await feed.updatePost(editingPost.id, content, link);
  };

  const handleTreasurePost = async (postId: string, isOwnPost: boolean) => {
    if (isOwnPost) {
      showToast('💡 You cannot treasure your own post', 'info');
      return;
    }
  ...
  return (
    <div className={styles.feedContainer}>
      <div className={styles.phoneFrame}>
        {/* Main Feed Content - NO Header, NO BottomNav here, they are in MainLayout */}
            ...
            {feed.loading && feed.posts.length === 0 ? (
              ...
              feed.posts.map(post => (
                <PostCard
                  key={post.id}
                  post={post}
                  onLike={() => feed.toggleLike(post.id, (post as any).is_own_post)}
                  onFollow={() => feed.toggleFollow(post.user_id, post.id)}
                  onReport={() => handleOpenReport(post)}
                  onComment={() => handleOpenComment(post)}
                  onTip={() => handleOpenTip(post)}
                  onShare={() => handleSharePost(post.id, (post as any).is_own_post)}
                  onTreasure={() => handleTreasurePost(post.id, (post as any).is_own_post)}
        ...
        {/* REMOVED: <BottomNav /> - MainLayout handles this */}
